# IA et LLM

Connaissances sur l'intelligence artificielle, les grands modèles de langage et les systèmes agents.

## Sous-domaines

| Sous-domaine | Description | Fiches |
|--------------|-------------|--------|
| [Agents](agents/_index.md) | Architecture d'agents IA, skills, orchestration, sécurité, déploiement enterprise | 7 |
| [Prompt Engineering](prompt-engineering/_index.md) | Techniques de prompting et optimisation | 0 |
| [RAG et Context](rag-et-context/_index.md) | Retrieval-augmented generation, mémoire agent, gestion du contexte | 2 |

---
*Mis à jour le 2026-02-11*
